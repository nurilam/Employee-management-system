...
<?php 
     ...
     $passwordPattern = "/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{8,}$/"; // At least 8 characters, one uppercase, one lowercase, one number, and one symbol
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(empty($_REQUEST["new_pass"])){
            ...
            if (!preg_match($passwordPattern, $new_pass)) {
                $new_passErr = " <p style='color:red'>* Password must be at least 8 characters long, with one uppercase letter, one lowercase letter, one number, and one special character </p>";
            }
        }
        ...
        if(!empty($old_pass) && !empty($new_pass) && !empty($confirm_pass) && empty($new_passErr)){
