...
<?php 
    ...
    $nameErr = $emailErr = $passErr = $salaryErr = $card_uidErr = "";
    $pass = "";
    ...
    if (!empty($name) && !empty($email) && !empty($pass) && !empty($salary) && !empty($card_uid)) {
        ...
        if ($email != $row["email"]) {  // Compare with the original email from the database
            ...
            if (mysqli_num_rows($r) > 0) {
                ...
                $sql = "UPDATE employee SET name = '$name', email = '$email', password = '$pass', dob = '$dob', gender = '$gender', salary = '$salary', card_uid = '$card_uid' WHERE id = $id";
                $result = mysqli_query($conn, $sql);
                if ($result) {
                    echo "<script>
            ...
            $sql = "UPDATE employee SET name = '$name', password = '$pass', dob = '$dob', gender = '$gender', salary = '$salary', card_uid = '$card_uid' WHERE id = $id";
            $result = mysqli_query($conn, $sql);
            if ($result) {
                echo "<script>
                    $(document).ready(function(){
